...
> **大多数 AI 工具的致命缺陷不是"不够聪明"，而是"太容易忘事"。**
> 每次新对话，你都要从头解释项目背景；AI 踩过的坑，下次还会再踩；精心调教的工作风格，重启就归零。

VibeSkills 为此构建了一套**四层记忆体系**，从会话内的短暂状态，到跨会话的长期知识图谱，确保知识不流失、决策可追溯、上下文永续传承。
...
> [!TIP]
> **可选外部扩展**：`mem0` 可作为个人偏好后端（输出风格、重复约束等），以 opt-in 方式软接入；`Letta` 提供记忆块映射与 Token 压力策略的契约词汇，但两者均**不会替代**上述四层的权威地位。

<br/>

### 专属记忆技能

除了底层架构，VibeSkills 还提供三个开箱即用的高层记忆技能：

<div align="center">

| 技能 | 定位 | 触发方式 |
|:---:|:---|:---|
| `knowledge-steward` | **知识管家**：一句话把对话中的灵感、踩坑、有效 Prompt 永久存入 Obsidian + GitHub，绝不让智力剩余价值随着上下文窗口消失 | "保存这个提示词" / "记录这个 Bug" / "save this insight" |
| `digital-brain` | **数字大脑**：结构化个人知识库，管理身份定位、内容创作、人脉网络、项目复盘——AI 的"第二大脑"完整实现 | 主动调用，适合建立个人知识操作系统 |
| `deepagent-memory-fold` | **上下文折叠**：长任务运行中自动将庞大上下文压缩为结构化的工作记忆 + 工具记忆 + 证据锚点，无缝交接给下一个 Agent 或新会话 | 长任务 context 临近上限时自动或手动触发 |
...
VibeSkills 的记忆层设计遵循严格的治理规则，防止记忆系统本身成为新的风险点：

<div align="center">

| 治理规则 | 说明 |
|:---|:---|
| **单一权威源** | 每种记忆需求只有一个负责组件，无双轨竞争 |
| **显式写入** | `Serena`（项目决策层）只在用户明确确认后才写入，绝不自动归因 |
| **禁用情节记忆** | `episodic-memory` 在 VCO 治理模式下永久禁用，防止隐式污染 |
| **mem0 有界接入** | 只允许记录个人偏好（风格、约束），禁止记录路由决策或项目真相 |
| **Kill Switch** | 任何外部记忆后端均有独立开关，一键降级至 `shadow`（只分类、不写入）模式 |
...
对话中的灵感/决策
       │
       ▼
  knowledge-steward ──→ Obsidian 笔记 + GitHub 备份
       │
       ▼
  Serena (项目层) ──→ 架构决策 / 技术约定
       │
       ▼
  Cognee (图谱层) ──→ 跨项目实体关联 / 长期知识积累
────────────────────────────────────────────────────────
新对话启动时，AI 自动载入项目记忆，无需重复介绍背景
```

<br/>

> _有了记忆层，每一次与 AI 的协作都不再是"从零开始"，而是站在上一次的肩膀上继续前进。_
